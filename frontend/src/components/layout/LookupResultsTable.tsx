...
export function LookupResultsTable({
  ...
  lookup: StorageQueryLookup | null;
  accessors: StorageAccessDescriptor[];
  chainId: string;
  onDismiss: () => void;
}) {
  ...
  return (
    <div className="mt-4" data-testid="lookup-result">
        ...
        <thead>
          <tr className="border-b border-gray-300">
            <th className={cn(dataTableHeadCellClass, 'w-[30%]')}>
              {structured ? 'Access / Field' : 'Access'}
            </th>
          ...
          {!structured && lookup.items[0] ? (
            <tr className="border-b border-gray-200">
              ...
              <td className={`${dataTableCellClass} hidden font-mono text-gray-500 sm:table-cell`}>
                {lookup.items[0].type_label}
              </td>
              <td className={`${dataTableCellClass} hidden sm:table-cell`}>
                <LocationCell item={lookup.items[0]} />
              </td>
              <td className={dataTableCellClass}>
                <ValueCell item={lookup.items[0]} chainId={chainId} />
              </td>
              <td className={`${dataTableCellClass} truncate sm:hidden`}>
                <LocationCell item={lookup.items[0]} compact />
              </td>
            </tr>
          ) : (
            <Fragment>
