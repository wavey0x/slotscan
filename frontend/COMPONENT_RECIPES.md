...
- Derive and render individual scalar storage values with `CompactValue`; it owns decoded/hex selection, compact display, full disclosure, and exact copy behavior. Render scalar before/after pairs with `StorageValueDiff`, use the layout-only `StorageViewValueCell` to translate storage-view availability states, and retain `StructuredValueDiff` for field trees. Use the lower-level `ValueDiff` only when composing non-storage or custom child content.
