...
- Derive and render every scalar storage value with `CompactValue`; it owns decoded/hex selection, compact display, full disclosure, and exact copy behavior. Compose before/after pairs with `ValueDiff` so both lines keep the same left edge, and retain `StructuredValueDiff` for field trees.
